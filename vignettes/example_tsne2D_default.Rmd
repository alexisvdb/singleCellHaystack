---
title: "Application of singleCellHaystack on 2D t-SNE coordinates"
...
params:
  path.to.rda.file: "../background/tabula_muris_micro_marrow_p7_2/tabula_muris_marrow_P7_2.rda"
---
...
```{r example0, include = FALSE}
load(file = "../background/tabula_muris_micro_marrow_p7_2/tabula_muris_marrow_P7_2.rda")
library(ggplot2) # need to load this for adding plot titles?
```

``` r
load("path/to/rda/file/tabula_muris_marrow_P7_2.rda")
ls()
# [1] "dat.detection"  "dat.expression" "dat.pca"        "dat.tsne"       "dat.umap" 
```
...
Let's have a look at the most biased genes. The most strongly biased gene is Alox5ap. We can plot the expression an detection of this gene using the `plot_gene_haystack` function. From the plots we can see that Alox5ap is not detected in the upper right and lower right group of cells.
```{r example4}
...
Cluster 1 contains 454 genes, and the smallest cluster (cluster 5) contains just 20 genes.
...
```{r example7, fig.height = 3, fig.width = 4}
for(cluster in 1:5){
...
```
The most biased genes, Alox5ap, was clustered into cluster 4. Comparing its expression pattern (see above) with that of each cluster, we can indeed see that it fits most closely with that of cluster 4. The pattern of cluster 5 (containing 20 genes) is rather different from all other clusters.
