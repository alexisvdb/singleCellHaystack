...
```{r example3}
res.pc10.adv <- haystack(x = dat.pca[,1:10], detection = dat.detection, method = "highD", use.advanced.sampling = general.detection)
```
...
```{r example4}
show_result_haystack(res.haystack = res.pc10.adv, n = 5)
# plotting detection of this gene
plot_gene_haystack(x = dat.tsne, gene = "Snca", expression = dat.detection)
# plotting log expression of this gene
plot_gene_haystack(x = dat.tsne, gene = "Snca", expression = log10(dat.expression)) 
```
...
Compare the pattern of Snca with that of the most biased gene in the default mode (see [here](articles/examplehighD_default.html)). The most biased gene found in the "default" mode is strongly biased, but is detected in cells that have in general many detected genes.
...
res.top <- show_result_haystack(res.haystack = res.pc10.adv, n = 1000)
...
```{r example8}
res.hc.clusters["Snca"] # the most biased gene is in cluster 5
```
