...
haystack_interface.matrix = function(coordinates, expression = NULL, detection = NULL,
  ...
  } else if(type == "2D" && !is.null(detection)){
    # 2D binary
    message("### calling haystack version 2D binary...")
    haystack_2D(x = coordinates[,1], y = coordinates[,2], detection = detection)

  } else if(type == "spatial" && !is.null(expression)){
    # spatial continuous
  ...
  } else if(type == "spatial" && !is.null(detection)){
